...
use crate::models::locations::{Location, LocationIn, LocationOut};
...
pub async fn get_locations(
    ...
    let exchange_rates = svc.get_locations_with_balance(limit, offset).await?;
...
pub async fn delete_location(
    ...
    let svc = LocationsService::new(&state);
    svc.delete_location(id).await.map_err(CommandsError::from)
}
...
pub async fn search_locations(
    state: tauri::State<'_, AppState>,
    query: String,
) -> Result<Vec<Location>, CommandsError> {
    let svc = LocationsService::new(&state);
